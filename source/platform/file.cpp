...
{
	file_t file = {};
	file.handle = SDL_RWFromFile(filepath, "rb");
	if (nullptr == file.handle)
	{
		log_line("error-sdl: SDL_RWFromFile {s}", SDL_GetError());
		return {};
	}

	log_line("file::open_read({s}) {p}", filepath, file.handle);
	return file;
}
...
{
	file_t file = {};
	file.handle = SDL_RWFromFile(filepath, "wb");
	if (nullptr == file.handle)
	{
		log_line("error-sdl: SDL_RWFromFile {s}", SDL_GetError());
		return {};
	...
	log_line("file::open_write({s}) {p}", filepath, file.handle);
	return file;
}

int32_t katha::file_t::length()
{
	if (nullptr == handle)
	{
		return 0;
...
uint32_t katha::file_t::read(void* buffer, const uint32_t bytes)
{
	...
	while (total_read < bytes)
	{
		...
		else if (0 == br)
		{
			...
			return static_cast<int>(total_read); // EOF
		}
...
uint32_t katha::file_t::write(const void* buffer, const uint32_t bytes)
{
	...
	while (total_write < bytes)
	{
		...
		else if (0 == bw)
		{
			...
			return static_cast<int>(total_write); // EOF
		}
