...
katha::result_e katha::gl_t::init(const config_t& config, SDL_Window* window)
{
	...
	if (ext_debug && config.enabled(feature_e::debug_graphics))
	{
...
{
	const GLuint src = static_cast<GLuint>(source.id);
	const GLuint dst = static_cast<GLuint>(target.id);
	if (!(src && dst))
	{
		log_line("warning-gl: blit_to_framebuffer(source={u:x}, target={u:x})",
			src,
			dst
		);
		return;
...
void katha::gl_t::draw_mesh(const mesh_t& mesh, const mat4& mvp)
{
	set_uniform_mat4(0, mvp.array());
	set_vertex_buffer(mesh.vertex_buffer, 0, 0, sizeof(vertex_t));
